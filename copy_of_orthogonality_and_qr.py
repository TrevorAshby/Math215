# -*- coding: utf-8 -*-
"""Copy of Orthogonality_and_QR.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1D-QQ7NLhAvyL0ljlf-F7N7XKs57YSEg3
...
def orthogonal_check(a,b):
  # Put your code here.
  cutoff = 1e-12
  if (np.dot(a,b) < cutoff) and (np.dot(a,b) > -1*cutoff):
    return True
  else:
    return False # Put your return value here.
...
def orth_set_check(vect_set):
  ...
  for i in range(len(vect_set)):
    if (orthogonal_check(vect_set[i-1],vect_set[i]) == False):
      return False
...
def normalize(v):
  # Put your code here.
  normV = (1/np.linalg.norm(v))*v
  return normV # Put your return value here.
...
def proj(a,b):
  # Put your code here.
  projba = (np.dot(a,b)/np.dot(b,b))*b
  return projba # Put your return value here.
...
def QR_decomposition(A):
  # Put your code here.
  x,y = np.shape(A)
  Q = np.transpose(np.array(gram_schmidt([A[:,j] for j in range(y)])))
  R = np.transpose(Q) @ A
...
def QR_solver(A,b):
  # Put your code here.
  Q,R = QR_decomposition(A)
  result = back_substitution(R,(np.transpose(Q)@b))
